...
var toolDescriptions = map[string]string{
	...
	"createPage": "Create a new page in a section with HTML content. Supports rich formatting, images, tables, and OneNote-specific elements.\n\nUser says \"Create a page called X with content Y\" → createPage(sectionID, title, htmlContent). Always validate title for illegal characters and suggest alternatives.\n\n**CONTENT FORMAT:** Accepts standard HTML with OneNote extensions. Use proper HTML structure with <html><head><body> tags. Images should be embedded as data URLs or use img tags with external URLs.\n\n**TITLE RESTRICTIONS:** Cannot contain: ? * \\ / : < > | & # ' ' % ~\nUse alternatives: & → \"and\", / → \"-\", : → \"-\", etc.\n\n**HTML GUIDELINES:**\n- Use semantic HTML elements (h1, h2, p, ul, ol, table)\n- Images: <img src=\"data:image/jpeg;base64,...\" /> or external URLs\n- Tables: Standard HTML table structure\n- Lists: <ul>/<ol> with <li> items\n- Emphasis: <strong>, <em>, <u> tags\n\n**EXAMPLES:**\n- Simple text: \"<html><body><h1>Title</h1><p>Content</p></body></html>\"\n- With image: \"<html><body><h1>Title</h1><img src='data:image/png;base64,iVB...' /><p>Caption</p></body></html>\"\n- With table: \"<html><body><table><tr><th>Header</th></tr><tr><td>Data</td></tr></table></body></html>\"",

	"updatePageContentAdvanced": "Update specific elements in a OneNote page using command-based targeting. Allows precise updates to individual elements without replacing entire page content.\n\n**WORKFLOW:** Step 1: getPageContent(pageID, forUpdate=true) to get current content with data-id attributes → Step 2: Identify target elements and their data-id values → Step 3: updatePageContentAdvanced with commands array.\n\n**TARGETING OPTIONS:**\n- data-id: \"data-id:element-123\" (most precise, get from getPageContent with forUpdate=true)\n- title: \"title\" (targets page title)\n- element selectors: \"h1\", \"p:first\", \"table\" (CSS-like selectors)\n\n**COMMANDS:**\n- append: Add content after target element\n- insert: Add content at specific position (after, before, inside)\n- replace: Replace target element completely\n- delete: Remove target element\n\n**POSITION OPTIONS (for insert):**\n- after: Insert after the target element\n- before: Insert before the target element  \n- inside: Insert inside the target element (at the end)\n\n**TABLE RESTRICTIONS:** Tables must be updated as complete units. You cannot update individual cells - you must replace the entire table element.\n\n**COMMAND FORMAT:**\n```json\n{\n  \"target\": \"data-id:element-123\",\n  \"action\": \"replace\",\n  \"content\": \"<p>New content</p>\",\n  \"position\": \"after\"\n}\n```\n\n**EXAMPLES:**\n- Replace title: {\"target\": \"title\", \"action\": \"replace\", \"content\": \"New Title\"}\n- Append paragraph: {\"target\": \"data-id:element-123\", \"action\": \"append\", \"content\": \"<p>New paragraph</p>\"}\n- Insert before element: {\"target\": \"data-id:element-456\", \"action\": \"insert\", \"content\": \"<h2>New Header</h2>\", \"position\": \"before\"}\n- Delete element: {\"target\": \"data-id:element-789\", \"action\": \"delete\"}",
	...
	"updatePageContent": "Replace the entire HTML content of a page. Use for complete page rewrites. For partial updates, use updatePageContentAdvanced instead. Tables must be replaced as complete HTML structures, and you should understand the style attributes from an existing table and include those in your update.",
}
